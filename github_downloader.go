...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("XEHCORD_DEV_INSTALL") == "1"
	...
	XehcordFile := XehcordDirectory

	stat, err := os.Stat(XehcordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		XehcordFile = path.Join(XehcordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(XehcordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Xehcord Install. Checking for hash...")

	re := regexp.MustCompile(`// Xehcord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(XehcordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", XehcordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", XehcordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(XehcordDirectory)
