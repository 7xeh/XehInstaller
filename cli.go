...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Xehcord")
	var updateFlag = flag.Bool("repair", false, "Repair Xehcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Xehcord")
	...
	if *versionFlag {
		fmt.Println("XehInstaller Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update XehInstaller' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Xehcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Xehcord will likely not work.")
}
