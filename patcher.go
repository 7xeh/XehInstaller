...
var XehcordDirectory string

func init() {
	if dir := os.Getenv("XEHCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using XEHCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../XehcordData")
		BaseDir = path.Join(dir, "..", "XehcordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Xehcord").UserConfig()
	}
	dir := os.Getenv("XEHCORD_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using XEHCORD_DIRECTORY")
		XehcordDirectory = dir
	} else {
		XehcordDirectory = path.Join(BaseDir, "xehcord.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, XehcordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", XehcordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+XehcordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + XehcordDirectory + ": " + err.Error())
		}
	}
	return nil
}
